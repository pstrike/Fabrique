...
<div <?php echo fabrique_a( $opts['item_attr'] ); ?> data-filter="<?php echo esc_attr( strtolower( implode( ', ', $opts['image']['tags'] ) ) ); ?>">
	<div <?php echo fabrique_a( $opts['body_attr'] ); ?>>
		...
		<?php if ( 'none' ===  $opts['on_image_click'] ) : ?>
			<div <?php echo fabrique_a( $opts['media_attr'] ); ?>>
		...
		<?php else : ?>
			...
			<?php else : ?>
				...
				<?php endif; ?>
			<?php endif; ?>
			<a <?php echo fabrique_a( $opts['media_attr'] ); ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>" data-index="<?php echo esc_attr( $opts['image_index'] ); ?>">
				<?php echo fabrique_template_external_media( $image ); ?>
			</a>
		<?php endif; ?>
	</div>

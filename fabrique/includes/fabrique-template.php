...
{
	$header_type = isset( $builder['header'] ) ? $builder['header'] : 'title';
	...
	if ( $query->have_posts() ) {
		while( $query->have_posts() ) {
			...
			if ( $categories && !is_wp_error( $categories ) ) {
				...
				foreach ( $term_names['name'] as $term_name ) {
...
function fabrique_template_external_media( $data = array() )
{
	...
	$output = '';
	$wrapper_class = array( 'fbq-media-wrapper external' );
	$image_attribute = '';
	$image_html = '';
	$attributes = array(
	...
	if ( 'none' !== $data['hover'] ) {
		$wrapper_class[] = 'anmt-image-' . $data['hover'];
	}
...
function fabrique_template_menu_cart( $in_navbar = true, $menu_position = 'top' )
{
	...
	if ( $woocommerce->cart ) {
		...
		$output .= '<li class="' . esc_attr( $menu_class ) . '">';
		$output .=    '<a href="' . esc_url( $woocommerce->cart->get_cart_url() ) . '" class="js-menu-cart"><i class="twf twf-' . esc_attr( fabrique_mod( 'navbar_cart_icon' ) ) . '"></i>';
